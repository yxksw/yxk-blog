...
function initializeAISummary() {
  ...
  // 如果已经有内容且不是"生成中"，说明已经动画完成，不再重复
  const currentText = summaryEl.textContent || ''
  const dataSummary = summaryEl.getAttribute('data-ai-summary') || ''
  if (currentText && currentText !== '生成中...' && currentText === dataSummary) {
    return
  }
...
;(function ensureInit() {
  ...
  document.addEventListener('swup:contentReplaced', initializeAISummary)
})()
